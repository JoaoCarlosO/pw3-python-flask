...
# Importamdo a biblioteca de segurança
...
def init_app(app):
    ...
    @app.before_request
    def check_auth():
        #rotas que não precisam de autenticação, permite o acesso:
        routes = ['hoem', 'login', 'caduser']
        #se a rota não requer autenticação, permite o acesso:
        if request.endpoint in routes or request.path.startswith('/static'):
            return
        #se o usu´srio nção estiver autenticado, redireciona para a página de login:
    ...
    def login():
        if request.method == 'POST':
            ...
            #Verificando  se emial e senha estão corretos
            if user and check_password_hash(user.senha, senha):
                #Aqui será criado a seesão para o usuário
            ...
            # Dados inválidos
                ...
                flash('Falha no Login. Verifique seu email e senha e tente novamente.', 'danger')
                return redirect(url_for('login'))
